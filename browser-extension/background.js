...
async function connectToServer() {
  if (!config.userId || !config.serverUrl) {
    console.log('配置不完整，无法连接:', { userId: config.userId, serverUrl: config.serverUrl });
    return;
  }
...
function shouldNotifyUser(data) {
  ...
  if (data.targetUsers && Array.isArray(data.targetUsers)) {
    return data.targetUsers.includes(config.userId) || 
           data.targetUsers.includes(config.userName);
  }
...
function buildNotificationMessage(data) {
  ...
  if (eventType === 'Merge Request Hook') {
    ...
    // 创建者
    const creator = data.user || objectAttrs.author?.name || objectAttrs.author?.username || rawData.user?.name || rawData.user_username || '-';
    messageParts.push(`创建者: ${creator}`);
    
    // 链接
    const url = objectAttrs.web_url || objectAttrs.url || eventData.webUrl || eventData.url || data.url || '#';
    if (url && url !== '#') {
...
async function showNotification(data) {
  ...
  // 提取 URL（组装正确的 GitLab URL）
  const rawData = data.raw || {};
  const objectAttrs = rawData.object_attributes || {};
  const eventData = data.data || {};
  
  // 组装 URL：从 target.git_http_url 提取基础 URL，从 url 提取路径
  let url = '#';
  if (objectAttrs.url && objectAttrs.target?.git_http_url) {
    try {
      // 从 git_http_url 提取基础 URL（协议、主机、端口）
      const gitHttpUrl = new URL(objectAttrs.target.git_http_url);
      const baseUrl = `${gitHttpUrl.protocol}//${gitHttpUrl.host}`;
      
      // 从 url 提取路径部分（去掉域名）
      const originalUrl = new URL(objectAttrs.url);
      ...
      url = objectAttrs.web_url || objectAttrs.url || eventData.webUrl || eventData.url || '#';
    }
  } else {
    // 如果没有 target.git_http_url，使用原始逻辑
    url = objectAttrs.web_url || objectAttrs.url || eventData.webUrl || eventData.url || '#';
  }
